...
The authors provide all the data and code in their github repository. From a fresh install of R, the software environment is easily set up (5 R packages have to be installed). The authors provide two types of script (one sequential, i.e. R script, and one step by step, i.e. like a jupyter notebook). The code could be executed successfully and all the paper figures could be reproduced.

\clearpage

# Reproducibility reviewer notes
1. Fork authors [repository](https://github.com/dataandcrowd/AGILE2022) into reproducible agile [repository](https://github.com/reproducible-agile/AGILE2022-16)

2. Prepare R environment
As mentioned by the authors in their github repository, the R version is 4.1.3. To maintain the proper functioning of potential others R projects, I used a docker image built by the [rocket-project](https://www.rocker-project.org/). The rocker/rstudio:4.1.3 image contains a Rstudio server with R version 4.1.3.

```{bash docker, eval=FALSE, size="tiny"}
sudo docker run -e ROOT=true -e PASSWORD=agile --rm -p 8787:8787 rocker/rstudio:4.1.3
```
When the dockers is started, connect to rstudio studio with a web browser: [http://localhost:8787](http://localhost:8787), with credentials as follow :
...
Using the docker's rstudio, go to the **"Terminal"** tab and add copy paste these instructions.
...
4. Install R package 
Using the docker's rstudio, go to the **"Console"** tab and add copy paste these instructions.
...
```
To fix this issue, "car" has to be installed
...
With this script (AGILE2022.R) 0 figure out of 6 were reproduced. After discussion with the authors, it appears that users can reproduce all the figures (6/6) by uncommenting the following lines: 
...
9. Run render function on index.qmd

5 more figures have been generated. They all correspond to the figures in the paper. Only the first figure of the paper (Histogram of the variables) is not reproducible with the script
...
- The README.md should be completed by:
  ...
  - ~~[The link to the website generated by quarto and hosted by github](https://dataandcrowd.github.io/AGILE2022/index.html)~~ changed by the authors
- The authors should add a section in the README.md explaining how to easily set up a R environment compatible with this code. Users could use a docker image built by the [rocket-project](https://www.rocker-project.org/) as used in this report.
