...
> Granell, C. (2022, May 10). Reproducilbility review of: A machine learning based approach on predicting usage efficiency of shared e-scooters using vehicle availability data,
...
Some scripts take some time. *I recommend adding estimated execution time to each script, as the ANN adn RF scripts took a long time to execute.*

The README file of the github repo is minimal, limited to the abstract and a link to the dataset on a  Drive folder. *I recommend adding usage information to the README file to associate each script to the expected output in the paper, and to add a `requeriment.txt` file or similar to indicate the required packages and their versions to run the analysis.*
